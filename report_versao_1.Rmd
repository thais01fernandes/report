...
tabela_completa_1 %>% 
  reactable(
    ...
    headerStyle = list(borderWidth = 3)),
    columns = list(
...
primeira_dimensao <- dados_completos %>% 
  ...
  select(1, 3) %>% 
  mutate(value = gsub("Positivo", "1 - Apoio do Supervisor, gestor ou liderança clínica para a segurança do paciente", value))
...
oitava_dimensao <- dados_completos %>% 
  ...
  filter(value == "Positivo") %>% 
  select(1, 3) %>% 
  mutate(value = gsub("Positivo", "8 - Reposta ao Erro", value))
...
dados_completos %>% 
  ...
  scale_fill_manual(values = c("Positivo" = "#68bd58", "Negativo" = "#c21f40", "Neutro" = "#e4c14e")) +  # Define as cores para cada categoria
